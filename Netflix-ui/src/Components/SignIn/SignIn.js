...
function SignIn() {
  ...
  // Password validation function
    ...
    const handleSubmit = (async(event) => {
      ...
      setPasswordError('');
        try{
          await signInWithEmailAndPassword(firebaseAuth, email, password).then(user =>{ localStorage.setItem('userEmail', email);navigate('/Netflix')});
          
      }catch(err){
          
          if(err.code==='auth/invalid-credential'){
              setEmailError("create a account")
          }else if(err.code=== 'auth/invalid-email'){
              setEmailError('Email is not valid')
          }else if(err.code==='auth/missing-password'){
              setPasswordError("Please enter your password")
          }else if(err.code==='auth/weak-password'){
            setPasswordError("password should contains 6 and 60 characters")
        }
        }
    });
    ...
    <div className="main">
     <div className='signNavbar'>
        <div className="navimage">
          <Link to='/'><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 111 30">
    ...
    <div className='SigninMain'>
        ...
        <div className="signinComponent">
            ...
            <form className='formstyle'onSubmit={handleSubmit}>
                <input type="text" placeholder='Email or Phonenumber' className='SigninPageInput' value={email} onChange={handleEmailChange}/>
                {emailError && <div className="error">{emailError}</div>}

               {/* Conditional rendering for password input based on showPasswordInput state */}
            {showPasswordInput && (
              <>
              <input type="Password" placeholder='Password' className='SigninPageInput' value={password} onChange={handlePasswordChange}/>
              {passwordError && <div className="error">{passwordError}</div>}
                ...
                {/* <input type="Password" placeholder='Password' className='SigninPageInput'/> */}
                <button type='Submit' className="buttonsection">Sign-in</button>
                <p>or</p>
                 {/* Button to toggle password input visibility */}
            <button type='button' onClick={togglePasswordInput} className="buttonsection" id='buttonone'>
              {showPasswordInput ? 'use a sign-in code' : 'Use Password'}
            </button>


                {/* <button type='Submit' className="buttonsection" id='buttonone'>Use Password</button> */}
                <div className="link">
                <Link to='' className='linking'>Forgot email or phone number?</Link>
                </div>
            </form>
            <footer className='footer'>
                ...
                <div className='Signupsection'>
                    <p>New to Netflix?</p>
                    <Link to='/' className='linking' id='linked'>Signup Now.</Link>
                </div>
